...
class Uploader:
 ...
 mysql = MYSQL(host="localhost",port=3306,user="root",pwd="",db="Automating")
                                        sql = "select count(1) from A_hosts_link where IP in ('%s')"%(I)
                                        print (sql)
                                        host_check = mysql.ExecQuery(sql) #得到{u'count': x}字典
                                        for i in host_check:
                                                if i.pop(u'count(1)') == 0:
                                                        #time.sleep(2)
                                                        #return self.write("0")
                                                        self.redirect("/host_cmd")
                                                else:
                                                        #mysql = MYSQL(host="localhost",port=3306,user="root",pwd="",db="Automating")
                                                        #host = mysql.ExecQuery("select * from A_hosts_link where IP = '%s'"%(I))
                                                        #print host
                                                        #return self.write("1") #主机已经录入准备执行命令
                                                        mysql = MYSQL(host="localhost",port=3306,user="root",pwd="",db="Automating")
                                                        host = mysql.ExecQuery("select * from A_hosts_link where IP = '%s'"%(I))
                                                        for P in host:
                                                                O = dict(P)
                                                                ...
                                                                #print cmd
                                                                for X in [ip]:
                                                                        a=threading.Thread(target=pc.ssh2,args=(X,port,username,passwd,cmd))
                                                                        a.start()
                                                                try:
...
if __name__=='__main__':
  ...
  try:
                        file = open("/root/yw/ziyuan.txt","r")
                        lines = file.readlines()
                        for i in lines:
                                i = i.strip('\t')
                                #print i 打印资产信息
                                return self.render('host_add.html',**{'result':i,'content':content})
                                  output = commands.getoutput('cat  /root/yw/log.txt ')
                        print output
                        #return self.render('host_add.html',**{'content':i,'result':result})
                        return self.render('host_add.html',**{'content':output,'result':result})
